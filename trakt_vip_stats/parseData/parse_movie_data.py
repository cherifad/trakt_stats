...
def users_top_10_watched_movies(year=None):
    ...
    with Session(engine) as session:
        if year is not None:
            # Filter by year using watched_at timestamps
            for id, runtime, watched_ats in session.exec(select(Movie.tmdb_id, Movie.runtime, Movie.watched_at)).fetchall():
                year_plays = 0
                for watched_at in watched_ats:
                    time = datetime.strptime(watched_at, "%Y-%m-%dT%H:%M:%S.%fZ").replace(tzinfo=timezone.utc)
                    if time.year == year:
                ...
                if year_plays > 0:
                    movies[id] = movies.get(id, 0) + (runtime * year_plays)
        else:
