    ...
    with Session(engine) as session:
        ...
        for watched_ats, title, tmdb_id_ in session.exec(select(Movie.watched_at, Movie.title, Movie.tmdb_id)).fetchall():
            for watched_at in watched_ats:
                try:
                    watched_at_dt = datetime.strptime(watched_at, "%Y-%m-%dT%H:%M:%S.%fZ").replace(tzinfo=timezone.utc)
                except Exception as e:
                    print(f"{e} for {tmdb_id}")
                    continue
                
                # If year parameter is provided, filter by that year
                if year is not None and watched_at_dt.year != year:
                    continue
                ...
                #     print(f"{e} for {tmdb_id}")
                #     continue
                if watched_at_dt < first_play:
                    first_play = watched_at_dt
                    movie_title = title
                    tmdb_id = tmdb_id_

    # If no movie found for the year, return None or empty data
    if movie_title is None:
        return {'movie': None, 'date': None, 'time': None, 'movie_logo': None}
