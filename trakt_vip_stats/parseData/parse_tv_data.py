...
def plays_by_time(year=None):
    ...
    with Session(engine) as session:
        for watched_ats in session.exec(select(Episode.watched_at)).fetchall():
            for watched_at in watched_ats:
                ...
                year_value = time.year
                month = time.strftime("%b")
                day = time.strftime("%a")
                hour = time.hour

                by_year[year_value] = by_year.get(year_value, 0) + 1    
    ...
    if by_year:
        ...
        for year_range in range(min_year, max_year):
            if year_range not in by_year.keys():
                by_year[year_range] = 0
...
def users_top_10_watched_shows(year=None):
    ...
    with Session(engine) as session:
        ...
        for show_id in shows_id:
            # Get episodes for this show, optionally filtered by year
            if year is not None:
                episodes = session.exec(
                    select(Episode.runtime, Episode.watched_at)
                    .where(Episode.tmdb_show_id == show_id)
                ).fetchall()
                
                # Filter episodes by year and sum runtimes
                watched_time = 0
                for runtime, watched_ats in episodes:
                    if watched_ats:
                        for watched_at in watched_ats:
                            time = datetime.strptime(watched_at, "%Y-%m-%dT%H:%M:%S.%fZ")
                            if time.year == year:
                                watched_time += runtime
                                break  # Count each episode only once per show
            else:
