...
def save():
    ...
    json_data['first_play_current_year'] = parse_other_data.first_play(year=datetime.now().year)
    json_data['pfp'] = parse_other_data.profile_picture()
    json_data['username'] = username

    trakt = {}
    trakt['most_watched_movies'] = parse_list_data.trakt_most_watched_movies()
    trakt['most_watched_shows'] = parse_list_data.trakt_most_watched_shows()
    json_data['trakt'] = trakt

    tv = {}
    tv['stats'] = parse_tv_data.tv_stats()
    tv['plays'] = parse_tv_data.plays_by_time()
    tv['plays_current_year'] = parse_tv_data.plays_by_time(year=datetime.now().year)
    tv['users_top_10'] = parse_tv_data.users_top_10_watched_shows()
    tv['users_top_10_current_year'] = parse_tv_data.users_top_10_watched_shows(year=datetime.now().year)
    ...
    movies['plays_current_year'] = parse_movie_data.plays_by_time(year=datetime.now().year)
    movies['users_top_10'] = parse_movie_data.users_top_10_watched_movies()
    movies['users_top_10_current_year'] = parse_movie_data.users_top_10_watched_movies(year=datetime.now().year)
    ...
    with open('all-time-stats.json', 'w') as file:
