...
export default function WrappedPage() {
  ...
  const mostActiveMonth = Object.entries(data.tv.plays.by_month).reduce(
    (max, [month, count]) => {
  ...
  const earlyBirdHours = Object.entries(data.tv.plays.by_hour).reduce(
    (sum, [hour, count]) => {
      const h = parseInt(hour);
  ...
  const peakHour = Object.entries(data.tv.plays.by_hour).reduce(
    (max, [hour, count]) =>
      count > max.count ? { hour: parseInt(hour), count } : max,
    { hour: 0, count: 0 }
  );

  const bingeDay = Object.entries(data.tv.plays.by_day_of_week).reduce(
  ...
  const slides = [
    // Welcome Slide
    {
      ...
      content: (
        <div className="flex flex-col items-center justify-center h-full text-center text-white">
          ...
          <h2
            className="text-4xl md:text-6xl font-bold mb-8 animate-fade-in-up"
            style={{ animationDelay: "0.2s" }}
          >
            {t("wrapped.title").split(" ").slice(2).join(" ")}
          </h2>
          <p
            className="text-2xl opacity-90 mb-12 animate-fade-in-up"
    ...
    // Total Plays
    {
      ...
      content: (
        <div className="flex flex-col items-center justify-center h-full text-center text-white">
          <p className="text-3xl mb-8 opacity-90 animate-fade-in-up">
      ...
      content: (
        <div className="flex flex-col items-center justify-center h-full text-center text-white">
          ...
          <h1 className="text-7xl md:text-8xl font-bold mb-8 animate-scale-in">
            {t(`days.${bingeDay.day}`) || bingeDay.day}
          </h1>
          <p className="text-3xl mb-4 opacity-90">
    ...
    // Final Summary
    {
      ...
      content: (
        <div className="flex flex-col items-center justify-center h-full text-center text-white px-8">
          <div className="wrapped-summary-container w-full max-w-6xl mx-auto p-8 rounded-3xl">
          ...
          <Link href="/">
            <Button
              size="lg"
              className="bg-white text-purple-600 hover:bg-gray-100 text-xl px-8 py-6 animate-pulse"
            >
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-black">
      ...
      {/* Navigation Buttons */}
      <div className="absolute bottom-8 left-0 right-0 flex justify-center gap-4 z-50 px-4 wrapped-nav-hidden">
        {currentSlide > 0 && (
          <Button
            onClick={prevSlide}
            variant="outline"
      ...
      {/* Slide Indicators */}
      <div className="absolute bottom-24 left-0 right-0 flex justify-center gap-2 z-50 wrapped-nav-hidden">
      ...
      {/* Skip Button */}
      <div className="absolute top-8 right-8 z-50 flex gap-4 wrapped-nav-hidden">
        {currentSlide === 0 && (
          <Button
            onClick={() => {
              setIsAnimating(true);
            ...
            variant="outline"
            className="bg-white/10 text-white border-white/20 hover:bg-white/20 backdrop-blur-sm text-sm"
          >
