...
export default function MoviesPage() {
    ...
    const hourData = Object.entries(movies.plays.by_hour).map(([hour, value]) => ({
        name: `${hour}:00`,
        value,
    }));

    const topCountries = getSortedByValue(movies.by_country, (val) => val, true).slice(0, 10);
        ...
        <div className="min-h-screen p-4 md:p-8 space-y-8">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <StatCard
                    ...
                    icon={Play}
                    description={`${movies.stats.plays.per_day.toFixed(1)} ${t('dashboard.perDay')}`}
                />
                <StatCard
                    title={t('movies.totalHours')}
                    value={formatNumber(movies.stats.hours.total)}
                    icon={Clock}
                    description={`${movies.stats.hours.per_month.toFixed(1)} ${t('dashboard.hoursPerMonth')}`}
                />
                <StatCard
            ...
            <div className="grid md:grid-cols-2 gap-6">
                <BarChart data={yearData} title={t('movies.moviesByYear')} maxItems={20} />
                <BarChart data={monthData} title={t('movies.moviesByMonth')} maxItems={12} />
            </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                            {Object.entries(movies.all_ratings).map(([rating, count]) => (
                                <div key={rating} className="text-center p-4 border rounded">
                                    ...
                                    <p className="text-sm text-muted-foreground">{rating} {t('movies.stars')}</p>
                                </div>
                            ))}
                        </div>
                    </CardContent>
                                    ...
                                    <div className="flex items-center gap-4 flex-1 ml-4">
                                        <div className="flex-1 bg-secondary rounded-full h-2 overflow-hidden">
                                            <div
                                                ...
                                                style={{
                                                    width: `${(item.value / Math.max(...Object.values(movies.by_released_year))) * 100}%`,
                                                }}
                                            />
                                        </div>
